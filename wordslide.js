...
const render = () => html`
    <textarea oninput=${e => {textvalue = e.currentTarget.value;update();}}>${textvalue}</textarea>
    <pre>${doupdate(textvalue)}</pre>
`;
...
function doupdate(television) {
    const res = [];
    for(const linev of television.split("\n")) {
        const linewords = [];
        for(const line of linev.split(" ")) {
            const rs0 = wordlist.filter(word => word.length === line.length);
            const rs1 = rs0.filter(c => !c.match(/[^a-zA-Z\-]/))
            const rs2 = rs1.filter(c => !c.match(/[^a-z\-]/))
            const rs3 = rs2.filter(c => !c.match(/[^a-z]/))
            const choices = rs3.length ? rs3 : rs2.length ? rs2 : rs1.length ? rs1 : rs0.length ? rs0 : [line];
            linewords.push(choices[0]);
        }
