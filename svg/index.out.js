...
(function(){document.body.removeChild(document.getElementById("jsloadnotice"));
  ...
  if(!SVG.supported){var alertTitle=document.createElement("h1");alertTitle.textContent="SVG Required";var alertContent=document.createElement("p");alertContent.textContent="Your browser does not appear to support SVG which is required for this program to run",document.body.appendChild(alertTitle),document.body.appendChild(alertContent);}

  var ctx = SVG('scan').size('100%', '100%');
  ...
  SVG.on(window, 'resize', function() {
    ...
    viewbox = {
      ...
      sx:ctx.node.clientWidth*viewbox.scale,
      sy:ctx.node.clientHeight*viewbox.scale,
      scale: viewbox.scale
    };
    updateViewbox();
  });
  
  ctx.node.addEventListener("mousedown",function(e){
    //console.log(e);
    if(e.button == 0)startPolyline(nax(e.clientX),nay(e.clientY));
    if(e.button == 1 || e.button == 2 || pressedKeys["17"])startScroll(e.clientX,e.clientY);
    if(e.button == 2 && pressedKeys["8"]){
      //console.log(e.path, e.path[0], e.path[0].nodeName);
      if(e.path[0].nodeName == "polyline")
        e.path[1].removeChild(e.path[0]);
    }
    if(e.button > 0) {e.preventDefault();return false;}
  });
  
  ctx.node.addEventListener("mousemove",function(e){
    //console.log(e);
    if(e.button == 0 && !pressedKeys["17"])updatePolyline(nax(e.clientX),nay(e.clientY));
    if(e.button == 1 || e.button == 2 || pressedKeys["17"])updateScroll(e.clientX,e.clientY);
    if(e.button > 0) {e.preventDefault();return false;}
  });
  
  ctx.node.addEventListener("mouseup",function(e){
    //console.log(e);
    //console.log("Ending Polyline stp1");
    if(e.button == 0)endPolyline(e.clientX,e.clientY);  
    if(e.button == 1 || e.button == 2 || pressedKeys["17"])endScroll(e.clientX,e.clientY);
    if(e.button > 0) {e.preventDefault();return false;}
  });
  
  var movement = false;
  var zooment = 0;
  ctx.node.addEventListener("touchstart",function(e){
    e.preventDefault();
    
    if(e.touches.length == 2){
      zooment = 0;
      console.log("oeh noez");
      if(polyline){ // better get rid of that stuff
        endPolyline();
        ctx.node.lastChild.remove();
      }
      e.touches[0].button = 1;
      movement = true;
  ...
  ctx.node.addEventListener("touchmove",function(e){
    e.preventDefault();
    if(e.touches.length == 2){
      var x1 = e.touches[0].clientX;
      var y1 = e.touches[0].clientY;
      ...
      zoomTo(-(zooment - c), ((x1-x2)/2+x1), ((y1-y2)/2+y1));
      //zoomTo(-(zooment - c), ctx.node.clientWidth/2,ctx.node.clientHeight/2);
      zooment = c;
    }else{
      zooment = 0;
      movement = false;
  ...
  ctx.node.addEventListener("touchend",function(e){
    zooment = 0;
    e = {button: 0};
    //console.log(e);
    //console.log("Ending Polyline stp1");
  ...
  ctx.node.addEventListener("mousewheel",function(e){
    //console.log(e.wheelDelta);
    zoomTo(e.wheelDelta,e.clientX,e.clientY);
  });
  ...
  var scrollpos = {};
  //var cb = ctx.node.getAttribute("viewBox").split(" ");
  //var viewbox = {x:cb[0],y:cb[1],sx:cb[2],sy:cb[3]};
  var viewbox = {x:0,y:0,sx:ctx.node.clientWidth,sy:ctx.node.clientHeight,scale:1};
  updateViewbox();
  
  var invertedScrolling = false;
  ...
  function updateScroll(x,y){
    if(scrollpos.x){
      //console.log(x, scrollpos.x, x - scrollpos.x);
      viewbox.x -= (invertedScrolling ? -1 : 1)*(x-scrollpos.x)*viewbox.scale;
      viewbox.y -= (invertedScrolling ? -1 : 1)*(y-scrollpos.y)*viewbox.scale;
      scrollpos.x = x;
      scrollpos.y = y;
      updateViewbox();
    }
  ...
  function zoomTo(count,x,y){
    ...
    viewbox.x -= (((wox - viewbox.x) * (-(1/cm))) - viewbox.x) + wox;
    viewbox.y -= (((woy - viewbox.y) * (-(1/cm))) - viewbox.y) + woy;
    
    updateSxSy();
  }
  
  function updateSxSy(){
    viewbox.sx = ctx.node.clientWidth * viewbox.scale;
    viewbox.sy = ctx.node.clientHeight * viewbox.scale;
    
    updateViewbox();
  }
  
  function endScroll(x,y){
    //console.log("ended scroll",ctx.node.getAttribute("viewBox"));
    scrollpos = {};
  }
  
  function updateViewbox(){
    ctx.node.setAttribute("viewBox",viewbox.x+" "+viewbox.y+" "+viewbox.sx+" "+viewbox.sy);
  }
  
  function nax(x){
    return normalizeCoord(x,"x");
  }
  
  function nay(y){
    return normalizeCoord(y,"y");
  }
  ...
  //var maxl = ctx.rect(maxX - minX, maxY - minY).x(minX).y(minY).back().fill('none').stroke({width: skw}); DISABLE visualliser
  
  function updateXY(x,y){
    updateX(x);
    updateY(y);
    
    // maxl.stroke({width:skw}).move(minX, minY).back().width(maxX - minX).height(maxY - minY);
  }
  
  function normalizeCoord(coord,val){
    //console.log(viewbox[val],coord,val);
    return ((coord*viewbox.scale + viewbox[val]));
  } 
  
  function viewViewbox(){
    ctx.node.setAttribute("viewBox",minX+" "+minY+" "+(maxX)+" "+maxY);
  }
  
  document.viewViewbox = viewViewbox;
  ...
  function startPolyline(x,y){
    polylinearr = [[x,y]];
    prevloc = [x,y];
    polyline = ctx.polyline(polylinearr).fill('none').stroke({ width: skw });
    
    updateXY(x,y);
  }
  
  function updatePolyline(x,y){
    if(!polyline) return;
    //if(!(math.distance([x,y],prevloc) > 3)) return;
    polylinearr.push([x,y]);
    polyline.plot(polylinearr);
    prevloc = [x,y];
  ...
  function endPolyline(x,y){
    //console.log("Ending Polyline");
    if(!polyline) return;
    
    var spable = [];
    polylinearr.forEach(function(pol){
      spable.push(pol[0]);
      spable.push(pol[1]);
    },this);
    
    var decode = smoothPath(spable);
    var nextelem = [];
    polylinearr = [];
    
    decode.forEach(function(elm){
      nextelem.push(elm);
      if(nextelem.length == 2){
        polylinearr.push(nextelem);
        nextelem = [];
      }
    });
    
    //console.log(decode.length, polylinearr.length * 2);
    
    polyline.animate(300).plot(polylinearr);
    //polyline.plot(polylinearr);
    polyline = undefined;
    polylinearr = undefined;
  }
  
  /*var rect = ctx.rect(100, 100).attr({ fill: '#f06' });*/
  
  /*var polyline = ctx.polyline([[0,0], [50,50], [50,100]]).fill('none').stroke({ width: 1 });
  console.log(polyline);
  //polyline.morphArray([[0,0], [100,50], [50,100]]);
  //polyline.array().push([[50,100]]);
  polyline.animate(1000).plot([[0,0], [100,50], [50,100]]);
  //polyline.animate(3000).plot([[0,0], [100,50], [50,100], [150,50], [200,50], [250,100], [300,50], [350,50]]); // use animate for smoothing
  
  console.log(ctx);*/
})();
},{"jquery":3,"jquery-mousewheel":2,"smooth-path":4,"svg.js":5}],2:[function(require,module,exports){
/*!
